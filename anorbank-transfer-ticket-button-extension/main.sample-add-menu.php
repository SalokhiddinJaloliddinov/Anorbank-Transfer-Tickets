...
class AddMenuSampleExtension implements iPopupMenuExtension
{
	...
	public static function EnumItems($iMenuId, $param)
	{
		...
		switch($iMenuId) // type of menu in which to add menu items
		{
			...
			$sOql = "SELECT URP_Profiles AS up JOIN URP_UserProfile AS uup ON uup.profileid=up.id JOIN User AS u ON uup.userid=u.id JOIN Person AS p ON u.contactid=p.id WHERE p.id = $iContactId";
			...
			if ($iProfileName == "Administrator" OR $iProfileName == "Accident Manager") {
				if ($param instanceof Ticket)
				{
					...
					$sModuleDir = basename(dirname(__FILE__));
					$sJSFileUrl = utils::GetAbsoluteUrlModulesRoot().$sModuleDir.'/js/sample.js';
					$aResult[] = new JSPopupMenuItem('_Custom_JS_', 'Трансфер', "MyCustomJSFunction('".addslashes($param->GetName())."')", array($sJSFileUrl));
				}
				if ($param instanceof DeliveryRequest)
				{
					break;
				}
			}
